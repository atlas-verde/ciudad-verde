  ...
  function(indicator_column, indicator_geom_col_label, indicator_y_axis_label, indicator_geom_col_fill, indicator_unit) {
    ...
    barplot_recreation_ggplot2 <-
      ...
      ggplot(aes(x = reorder(
        !!sym(COLUMN_CANTON_NAME), -indicator_column
      ), y = indicator_column)) +
      geom_col(aes(
        text = paste0(
          "Cantón: ",
          !!sym(COLUMN_CANTON_NAME),
          "\n",
...
COLUMN_WILDPROTECTEDAREAS_PERCENTAGE <- cantons$porc_asp
COLUMN_GREENBLUEINFRAESTRUCTURE_PERCENTAGE <- cantons$porc_tv
COLUMN_RIVERSIDEVEGETATION_PERCENTAGE <- cantons$porc_veg_r

COLUMN_CARBON_STOCK <- cantons$stock_c
COLUMN_CARBON_DENSITY <- cantons$dens_c
COLUMN_SOILBIOTICINDEX <- cantons$IBS
COLUMN_LANDSURFACE_TEMPERATURE <- cantons$lst_media

COLUMN_POPULATIONINTERNETACCESS_PERCENTAGE <- cantons$PORC_INTER

COLUMN_GREENAREA_AREA <- cantons$supverd.ha
COLUMN_TRAILS_LENGTH <- cantons$km_sendero

COLUMN_POPULATIONINFORMALSETTLEMENTS_PERCENTAGE <- cantons$porc_ai
...
QUANTILES_WILDPROTECTEDAREAS_PERCENTAGE_VALUES <- fivenum(COLUMN_WILDPROTECTEDAREAS_PERCENTAGE)
QUANTILES_WILDPROTECTEDAREAS_PERCENTAGE_LABELS <- get_quartiles_labels(QUANTILES_WILDPROTECTEDAREAS_PERCENTAGE_VALUES, UNIT_WILDPROTECTEDAREAS_PERCENTAGE)
  ...
  output$map_urbanbiodiversity <- renderLeaflet({
    if (input$radiobuttons_indicators_urbanbiodiversity == INDICATOR_WILDPROTECTEDAREAS_PERCENTAGE) {
      create_map(
        ...
        ~ PALETTE_WILDPROTECTEDAREAS_PERCENTAGE(porc_asp),
      ...
      create_map(
        ...
        ~ PALETTE_RIVERSIDEVEGETATION_PERCENTAGE(porc_veg_r),
  ...
  output$map_environmentalquality <- renderLeaflet({
    if (input$radiobuttons_indicators_environmentalquality == INDICATOR_CARBON_STOCK) {
      ...
      create_map(
        ...
        ~ PALETTE_LANDSURFACE_TEMPERATURE(lst_media),
  ...
  output$map_smartcity <- renderLeaflet({
    if (input$radiobuttons_indicators_smartcity == INDICATOR_POPULATIONINTERNETACCESS_PERCENTAGE) {
      create_map(
        ...
        ~ PALETTE_POPULATIONINTERNETACCESS_PERCENTAGE(PORC_INTER),
  ...
  output$map_recreation <- renderLeaflet({
    if (input$radiobuttons_indicators_recreation == INDICATOR_GREENAREA_AREA) {
      create_map(
        ...
        ~ PALETTE_GREENAREA_AREA(supverd.ha),
  ...
  output$map_sustainableurbandevelopment  <- renderLeaflet({
    if (input$radiobuttons_indicators_sustainableurbandevelopment == INDICATOR_POPULATIONINFORMALSETTLEMENTS_PERCENTAGE) {
      create_map(
        ...
        ~ PALETTE_POPULATIONINFORMALSETTLEMENTS_PERCENTAGE(porc_ai),
